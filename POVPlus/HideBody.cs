...
namespace POVPlus
{
    ...
    public static unsafe class HideBody
    {
        ...
        private static bool AutoFaceTargetConfigCache;
        ...
        private static void GetCameraPositionDetour(GameCamera* camera, GameObject* target, Vector3* position, Bool swapPerson)
        {
            ...
            if (!Plugin.P.Configuration.Setting_ModEnabled)
            {
                ...
                if (Service.GameConfig.UiControl.GetBool("AutoFaceTargetOnAction") != AutoFaceTargetConfigCache)
                    Service.GameConfig.UiControl.Set("AutoFaceTargetOnAction", AutoFaceTargetConfigCache);
                return;
            }
            ...
            if (swapPerson)
            {
                switch (camera->mode)
                {
                    case 1:
                        ...
                        if (Plugin.P.Configuration.Setting_FirstPersonOnly) //1 is third person (If you are third person and third person ISNT TICKED
                        {
                            camera->VTable.getCameraPosition.Original(camera, target, position, swapPerson);
        ...
        public static void Initialize()
        {
            ...
            AutoFaceTargetConfigCache = Service.GameConfig.UiControl.GetBool("AutoFaceTargetOnAction");
